    ...
    with container:
        ...
        with admin_subtab[0]:
            ...
            try:
                ...
                if not users_df.empty:
                    st.dataframe(users_df[['user_name','role','location']], width='stretch', hide_index=True)
                    st.divider()

                    # Update Role
                    with st.form("role_update_form"):
                        target_user = st.selectbox("Select User", users_df['user_name'].unique())
                    ...
                    with st.form("loc_add_form"):
                        target_user_loc = st.selectbox("Select User", users_df['user_name'].unique(), key="loc_add_user")
                        new_loc = st.selectbox("Assign Location", ["Pv","Canape-Vert"], key="loc_add_loc")
                    ...
                    with st.form("loc_remove_form"):
                        target_user_loc = st.selectbox("Select User", users_df['user_name'].unique(), key="loc_remove_user")
                        existing_locs = users_df[users_df['user_name']==target_user_loc]['location'].unique().tolist()
                        remove_loc = st.selectbox("Select Location to Remove", existing_locs, key="loc_remove_loc")
                        if st.form_submit_button("Remove Location"):
                    ...
                    with st.form("add_user_form", clear_on_submit=True):
                        ...
                        new_role = st.selectbox("Role", ["Admin","Manager","Staff"], key="new_user_role")
                        new_locations = st.multiselect("Assign Locations", ["Pv","Canape-Vert"], key="new_user_locs")
                        default_password = st.text_input("Default Password", value="DressupHT@2026!", type="password", key="new_user_pwd")
        ...
        with admin_subtab[2]:
            ...
            with col2:
                ...
                if master_inventory is not None:
                    st.metric("Total Items in System", len(master_inventory))
