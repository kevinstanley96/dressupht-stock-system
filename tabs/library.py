...
def render_tab(container, supabase, username, role, loc_list, t):
    with container:
        ...
        if not master_inventory.empty:
            ...
            page_size = c5.selectbox("Rows per page", [20, 50, 100], index=1)
            total_pages = max(1, (len(disp_df) // page_size) + (1 if len(disp_df) % page_size else 0))
            page_number = c6.number_input("Page", min_value=1, max_value=total_pages, value=1)
            ...
            # Compact view toggle
            compact_view = st.checkbox("📱 Compact view (mobile-friendly)", value=False, key="library_compact")

            # Location toggle for multi-location users
            show_location = False
            if role != "Staff" and len(loc_list) > 1:
                show_location = st.checkbox("Show Location column", value=False)
            ...
            safe_dataframe(page_df, cols, "No items found.")
