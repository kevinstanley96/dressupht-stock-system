...
def render_tab(container, supabase, username, role, loc_list, t, master_inventory=None):
    with container:
        ...
        t_search = st.text_input("🔍 Search PV Inventory", placeholder="Search by SKU or Name...", key="transfer_search").lower()
        pv_inventory = master_inventory[master_inventory['Location'] == "Pv"].copy() if master_inventory is not None else pd.DataFrame()

        if t_search and not pv_inventory.empty:
